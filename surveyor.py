...
cmd_parser.add_argument('--simple-rep', help='simpleRepeats file from UCSC table browser.')
...
if cmd_args.simple_rep != None:
    config_file.write("simple_rep %s\n" % cmd_args.simple_rep)
...
for i in range(1, num_contigs+1):
    for dt in ("OW", "SI", "LI"):
        fname = "%s/%d-%s.bam" % (workspace, i, dt)
        sorted_fname = "%s/%d-%s.sorted.bam" % (workspace, i, dt)
        print "Sorting", fname
        pysam.sort("-@", str(cmd_args.threads), "-o", sorted_fname, fname)
        os.rename(sorted_fname, fname)
