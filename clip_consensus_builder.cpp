...
void build_clip_consensuses(int id, int contig_id) {
    ...
    // find for each lc anchor the best rc anchor which is close (may represent an insertion)
    std::unordered_map<consensus_t*, consensus_t*> rc_anchors_by_lc_anchor;
    for (int i = 0; i < clip_consensuses.size(); i++) {
        consensus_t* rc_anchor = clip_consensuses[i];
        if (rc_anchor->left_clipped || rc_anchor->sc_reads < 5) continue;

        // look for strongest lc cluster within readlen/2 of rc cluster
        consensus_t* best_lc_anchor = NULL;
        int j;
        // find range of good left clips (- readlen/2 + maxSCDistance)
        for (j = i+1; j < clip_consensuses.size(); j++) {
            consensus_t* lc_anchor = clip_consensuses[j];
            if (lc_anchor->clipped_pos() - rc_anchor->clipped_pos() > config.max_sc_dist) break;
            if (!lc_anchor->left_clipped || lc_anchor->sc_reads < 5) continue;

            if (best_lc_anchor == NULL || best_lc_anchor->sc_reads <= lc_anchor->sc_reads) {
                best_lc_anchor = lc_anchor;
            }
        }
        for (j = i-1; j >= 0; j--) {
            consensus_t* lc_anchor = clip_consensuses[j];
            if (rc_anchor->clipped_pos() - lc_anchor->clipped_pos() > config.read_len/2) break;
            if (!lc_anchor->left_clipped || lc_anchor->sc_reads < 5) continue;
        ...
        if (best_lc_anchor != NULL) {
            rc_anchors_by_lc_anchor[best_lc_anchor] = rc_anchor;
        }
    }

    // align rc-lc pairs and check for possible INS
    int CLIP_ALIGN_END_TOLERANCE = 2;

    std::ofstream si_writer(workdir + "/workspace/" + std::to_string(contig_id) + "-SI.txt");
    ...
    for (auto& e : rc_anchors_by_lc_anchor) {
        ...
        if ((alignment.query_begin > CLIP_ALIGN_END_TOLERANCE && alignment.ref_begin > CLIP_ALIGN_END_TOLERANCE) ||
            (alignment.query_end < rc_anchor->consensus.length()-1-CLIP_ALIGN_END_TOLERANCE &&
             alignment.ref_end < lc_anchor->consensus.length()-1-CLIP_ALIGN_END_TOLERANCE)) {
            continue;
        }

        auto alignment_len = [](StripedSmithWaterman::Alignment& a) {
            return std::min(a.ref_end-a.ref_begin, a.query_end-a.query_begin);
        };

        int rc_abegin = alignment.query_begin, rc_aend = alignment.query_end,
            lc_abegin = alignment.ref_begin, lc_aend = alignment.ref_end;
        if (alignment_len(alignment) >= 30) {
            int newlen = alignment_len(alignment);
            if (lc_abegin <= CLIP_ALIGN_END_TOLERANCE && rc_aend >= rc_anchor->consensus.length()-1-CLIP_ALIGN_END_TOLERANCE) {
                newlen = lc_anchor->consensus.length() + rc_anchor->consensus.length() - alignment_len(alignment);
            } else if (rc_abegin <= CLIP_ALIGN_END_TOLERANCE &&
                    lc_aend >= rc_anchor->consensus.length()-1-CLIP_ALIGN_END_TOLERANCE) {
                // TODO: realign extra clip portions to anchors
            }

            if (newlen < config.min_sv_len) continue;

            int l_pos, r_pos;
            consensus_t* l_anchor,* r_anchor;
            if (lc_anchor->clipped_pos() < rc_anchor->clipped_pos()) {
                l_pos = lc_anchor->clipped_pos();
                r_pos = rc_anchor->clipped_pos();
                l_anchor = lc_anchor;
                r_anchor = rc_anchor;
            } else {
                l_pos = rc_anchor->clipped_pos();
                r_pos = lc_anchor->clipped_pos();
                l_anchor = rc_anchor;
                r_anchor = lc_anchor;
            }
            ...
            cluster_t c(a1, a2, DISC_TYPES.SI, 0);
            si_writer << c.to_str() << "\n";
        }
    }
    si_writer.close();
